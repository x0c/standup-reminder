...
5. 发现离开：起点清零，日志「检测到离开（原因）」。原因取值：`screensaver` / `locked` / `loginwindow` / `wake` 等。
6. 系统唤醒时间变化：视为离开并清零（合盖休眠走这条，**不是**热角屏保路径）。
...
| `tests/test.sh` | 前台名解析、ioreg 锁屏键、本机解锁判定 | 1 文件 |
...
| 改到点打断 | 同上 | `fire_reminder` / `hide_all_windows` / `show_reminder` / `start_screensaver` | 演练模式只写日志 |
...
| `unlocked_since` | 本段连续使用起点（unix 秒） | 离开或唤醒必须写 0 |
| `elapsed` | 已连续使用秒数 | `status` 用来算「还有多久」 |
| `last_reason` | 最近一次心跳/离开原因 | 验收热角时看是不是 `screensaver`/`locked` |
...
- **AI 易错点** 【消歧】「系统唤醒」vs「屏保/锁屏离开」：合盖休眠会打「检测到系统唤醒，计时已重置」，看起来像清零成功，**不能**当作热角/锁屏路径已修好。热角通过必须看到「检测到离开（screensaver）」或「（locked）」。
- 【叫法统一】正文用「连续使用 / 离开 / 提醒打断」；日志里仍是「解锁 / 离开 / 计时中」。
- 【隐性依赖】改离开判断后必须本机热角验收（停满 20 秒），单测不够。
- 【低置信度】「需要密码才进锁屏」的延迟秒数因系统设置而异；热角后可能先是屏保、再变锁屏。证据：用户用热角进入，日志原因是 `screensaver`。待确认：仅锁屏热角（不经屏保）是否稳定打出 `locked`。

## §7 常见易忽略条件与验证路径

- 改离开判断后：`bash tests/test.sh`（含 ioreg 有/无锁屏键、本机应判定已解锁）。
- 本机实装后：`standup-reminder doctor` 须显示正在计时；日志须有「检测到解锁，开始计时」。
- 热角路径：触发角开屏保，**画面上停满 20 秒**再解锁。日志验证：
  ```bash
  grep -E '离开|解锁|唤醒' "$HOME/Library/Application Support/standup-reminder/run.log" | tail -10
  ```
  须出现 `检测到离开（screensaver）` 或 `（locked）`，随后 `检测到解锁，开始计时`。只有 `系统唤醒` 则验收失败。
- 停不够 20 秒会被 10 秒一轮漏掉，禁止据此报修好或没修好。
...
- 用户 / 资料补充：热角屏保是真实离开路径；停满 20 秒才算数。来源：用户补充（2026-08-15 本机验证通过）。
